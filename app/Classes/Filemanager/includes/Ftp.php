...
namespace App\Classes\Filemanager\includes;
...
class Ftp
{
    ...
    public function __construct(array $config)
    {
        ...
        $this->_path = isset($config['path']) ? $config['path'] : $this->_path;
    ...
    public function upload($localPath, $remotePath, $mode = 'auto', $permissions = null)
    {
        ...
        $remotePath = $this->_path . $remotePath;
    ...
    {
        $oldFile = $this->_path . $oldFile;
        $newFile = $this->_path . $newFile;
    ...
    public function mirror($localPath, $remotePath)
    {
        ...
        $localPath = $this->_path . $localPath;
        $remotePath = $this->_path . $remotePath;
     ...
     * @return    array
     */
    public function listFilesRaw($path = '', $onlyDirectories = false, array $excludeIf = ['.', '..'], $includeExtensionFileOnlyIf = array())
    {
        if (!$this->_isConnected()) {
            return false;
        }
        $result = false;
        $dir = $this->_path . $path;
        if (count(explode('/', $path)) <= 3 && @ftp_chdir($this->_conn, $dir) === false) {
            $this->mkdir($path); // Create dir for special ID of user/shop
        }
        if (is_array($children = @ftp_rawlist($this->_conn, $dir))) {
            $items = [];
            $folder = [];
