...
class ReportAlertPDF extends ReportPDF
{
    /**
     * @param string $uuid
     * @return string
    ...
    public function generateReport(string $uuid, $forceupdate = false, $alert = null)
    {
        ...
        if ($alert && $alert->type) {
            switch ($alert->type) {
                case "wave":
                    $score_item = 'program';
                    $score_filter = $target->program_id;
                    break;
                case "mission":
                    $score_item = 'wave_target';
                    $score_filter = $target->id;
                    break;
                default:
            ...
            $questions = $questions->where($score_item . "_id", $score_filter);
        }
        ...
        $pdf->SetXY(
            ...
            $this->yCoordCellStartPosition + ($this->yCoordStepBetweenCellsWithTitle * $this->iCellBlockCounter) - $this->yCoordStepBetweenCellAndItsTitle
        ); // 185
        ...
        if ($mission['date_visit']) {
            $visit_date = (isset($target['visit_date']))
                ? Carbon::parse($target['visit_date'])
                : Carbon::parse($target['date_status']);

            $visit_date = $visit_date->format('d/m/Y');
            $pdf = $this->setDateVisit($pdf, $visit_date);
        }
        ...
        // return report
        $pdfName = $this->returnReport($target, $pdf, $mission, ReportPdfFactory::TYPE_ALERT, $forceupdate);
