...
class ReportStandardPDF extends ReportPDF
{
    /**
     * @param string $uuid
     * @return string
    ...
    public function generateReport(string $uuid, $forceupdate = false, $alert = null)
    {
        ...
        //update global score in ddb
        $wt = WaveTarget::find($target['id']);
        if (intval($wt->score) !== intval($global)) {
            $wt->score = $global;
            $wt->save();
        }
        ...
        # cell question on dynamic question on first page
        $questions = \DB::table('show_question_criteria_report_pdf')
            ->select('*')
            ->where('is_visible_on_first_page_report', true)
            ->where('user_id', $target['user_id'])
            ->where('uuid', $uuid)
            ->get();
        $pdf = $this->setFirstPageQuestion($pdf, $questions);
