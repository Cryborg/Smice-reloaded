...
class ReportSynteticPDF extends ReportPDF
{
    /**
     * @param string $uuid
     * @return string
    ...
    public function generateReport(string $uuid, $forceupdate = false, $alert = null)
    {
        ...
        $pdf = $this->setSequencesAndQuestions($pdf, $questionsGroupedBySequence);

        //$pdf = $this->setSequences($pdf, $target, $questionsGroupedBySequence);
        // add signature
        $pdf = $this->setSignature($pdf, $target);
        // return report
        $pdfName = $this->returnReport($target, $pdf, $mission, ReportPdfFactory::TYPE_SYNTETIC, $forceupdate);
